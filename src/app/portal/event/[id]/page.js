...
export default function Home({ params }) {
    ...
    const checkAuth = async () => {
      try {
        ...
        if (user) {
          ...
          fetchRegisteredEvents()
            .then((res) => {
              setUserEventDetails(res.data?.events || []);
            })
            .catch((err) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const res1 = await fetchEventById(id);
        // New API returns event in res1.data.event
        const eventData = res1.data.event || res1.data;
        ...
        if (eventData) {
          ...
          // Transform contacts array to contact_name_1, contact_mobile_1 format
          const transformedEvent = {
            ...eventData,
            date: dayOfMonth,
            // Map contacts array to flat structure
            contact_name_1: eventData.contacts?.[0]?.name || "",
            contact_mobile_1: eventData.contacts?.[0]?.mobile || "",
            contact_name_2: eventData.contacts?.[1]?.name || "",
            contact_mobile_2: eventData.contacts?.[1]?.mobile || "",
            // Map rounds array to flat structure
            round_title_1: eventData.rounds?.[0]?.title || "",
            round_desc_1: eventData.rounds?.[0]?.description || "",
            round_title_2: eventData.rounds?.[1]?.title || "",
            round_desc_2: eventData.rounds?.[1]?.description || "",
            round_title_3: eventData.rounds?.[2]?.title || "",
            round_desc_3: eventData.rounds?.[2]?.description || "",
            round_title_4: eventData.rounds?.[3]?.title || "",
            round_desc_4: eventData.rounds?.[3]?.description || "",
            // Ensure teamSize is string for comparison
            teamSize: eventData.teamSize?.toString() || "1",
          };
          setEventDetail(transformedEvent);
        }
      } catch (error) {
  ...
  const handleRegister = async () => {
    ...
    try {
      // Fetch fresh profile to check auth status
      const { user } = await authService.getProfile();
      ...
      // If profile fetch fails, show login overlay
  ...
  return !eventDetail ? (
    ...
    <div
      className={`flex-1 h-[calc(100vh-40px)] lg:h-screen mt-10 flex flex-col overflow-y-auto ${eventDetail.category === "Gold"
        ? "bg-[#FAF8C8]"
  ...
  lg:font-normal lg:tracking-wider lg:text-sm font-poppins`}
    ...
    ${eventDetail.category === "Gold"
              ...
              <h2
                className={`font-bold text-black   ${eventDetail.eventName.length > 15
            ...
            <div className="flex flex-col gap-4">
              ...
              <div className="flex flex-col w-full gap-4">
                ...
                <div className="flex items-center gap-4">
                  ...
                  <p
                    className={`text-xl ${eventDetail.hall.length > 20 ? "md:text-sm" : "lg:text-xl"
            ...
            >
              <div className="flex flex-col items-start justify-between lg:flex-row">
                <div>
                  ...
                  <h3
                    className={`font-bold mt-1 text-black ${eventDetail.eventName.length > 15
                      ? "text-3xl"
                      : "text-5xl"
                      }`}
                  >
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-70">
          <div className="bg-white rounded-lg p-8 max-w-md w-full mx-4 text-center">
            ...
            <div className="flex flex-col gap-3">
              <button
                className="w-full px-6 py-3 text-white bg-black hover:bg-gray-800 font-semibold rounded-lg transition-colors"
                onClick={() => router.push("/auth?type=login")}
              >
                Login
              </button>
              <button
                className="w-full px-6 py-3 text-black bg-gray-200 hover:bg-gray-300 font-semibold rounded-lg transition-colors"
